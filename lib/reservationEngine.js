 ...
 */
export async function createReservation(userId, dateStr, client = prisma) {
  ...
  // Vérifier si le parking est fermé pour cette date
  const closure = await prisma.parkingClosure.findFirst({
    where: {
  ...
  if (closure) {
    return {
      success: false,
      error: `Le parking est fermé du ${formatDateFR(closure.startDate)} au ${formatDateFR(closure.endDate)} : ${closure.reason}`,
    };
  }
  ...
  if (user.isPmr && user.pmrValidatedAt) {
    ...
    const totalPmrSpots = await client.parkingSpot.count({
      where: { type: 'PMR', isActive: true },
    });
    const reservedPmrSpots = await client.reservation.count({
      where: {
        date: dateStr,
        status: 'CONFIRMED',
        spot: { type: 'PMR' },
  ...
  const consecutiveDays = await countConsecutiveDays(client, userId, dateStr);
  ...
  if (user.isPmr && user.pmrValidatedAt && !usedPmrPriority) {
    ...
    spot = await findAvailableSpot(client, dateStr, 'PMR');
  }

  if (!spot) {
    // Place standard
    spot = await findAvailableSpot(client, dateStr, 'STANDARD');
  }
 ...
 */
export async function cancelReservation(userId, reservationId, client = prisma) {
 ...
 */
export async function getAvailability(dateStr, client = prisma) {
  const allSpots = await client.parkingSpot.findMany({
    where: { isActive: true },
    orderBy: { spotNumber: 'asc' },
  });

  const reservedSpotIds = await client.reservation.findMany({
...
async function findAvailableSpot(client, dateStr, type) {
  const reservedSpotIds = await client.reservation.findMany({
...
async function countConsecutiveDays(client, userId, targetDateStr) {
  ...
  for (let i = 0; i < MAX_CONSECUTIVE_DAYS + 1; i++) {
    ...
    const hasReservation = await client.reservation.findFirst({
      where: {
        userId,
        date: dateStr,
        status: 'CONFIRMED',
